# N3-13 — Gerúndio e Infinitivo Avançado (Gerunds & Infinitives)

> **Nível:** N3 Intermediário (B1+/B2) · **Trilha:** Gramática
> **Requer:** N2-07 Verbos + gerúndio ou infinitivo (stop doing / stop to do)
> **Destrava:** — (tema terminal)

## Em 1 frase

Quando dois verbos se encontram, o segundo vira **-ing** ou **to + verbo** dependendo do primeiro — e no nível avançado entram os verbos que **mudam de sentido** (*remember, try, regret...*), o padrão **verbo + pessoa + to** (*I want you to come*) e a regra de ouro: **depois de preposição, sempre -ing**.

## Explicação completa

### Recapitulando a base (N2-07)

| Grupo | Verbos | Exemplo |
|-------|--------|---------|
| **+ -ing** | enjoy, finish, avoid, mind, keep, suggest, practise, consider, miss | *I **enjoy living** in Brisbane.* — Gosto de morar em Brisbane. |
| **+ to** | want, decide, hope, plan, learn, promise, agree, offer, refuse, manage, afford | *We **decided to move** to Australia.* — Decidimos nos mudar para a Austrália. |
| **Tanto faz** (mesmo sentido) | like, love, hate, start, begin, continue, prefer | *It **started raining** / **started to rain**.* — Começou a chover. |

> ⚠️ Exceção do grupo "tanto faz": com **would**, só infinitivo — *I **would like to visit** Sydney* (nunca ~~would like visiting~~). *Like doing* = gostar em geral; *would like to do* = querer agora.

### Os verbos que mudam de sentido — o coração do N3

Aqui **-ing vs to** muda o **significado** da frase, não só a forma:

| Verbo | + -ing | + to |
|-------|--------|------|
| **remember** | *I **remember locking** the door.* — Lembro de ter trancado (memória do passado). | *I **remembered to lock** the door.* — Lembrei de trancar (não esqueci da tarefa). |
| **forget** | *I'll never **forget seeing** the Opera House.* — Nunca vou esquecer de ter visto. | *Don't **forget to bring** your passport.* — Não esqueça de trazer. |
| **stop** | *She **stopped drinking** coffee.* — Parou de tomar (largou o hábito). | *She **stopped to drink** coffee.* — Parou (o que fazia) **para** tomar um café. |
| **try** | ***Try studying** in the morning.* — Experimente estudar de manhã (teste, pode funcionar). | *I **tried to study**, but I was too tired.* — Tentei estudar (me esforcei, era difícil). |
| **regret** | *I **regret not studying** English earlier.* — Me arrependo de não ter estudado antes. | *We **regret to inform** you...* — Lamentamos informar... (formal, dar má notícia). |
| **go on** | *He **went on talking** for an hour.* — Continuou falando (a mesma coisa). | *She **went on to become** a manager.* — Depois, passou a ser gerente (mudou de etapa). |
| **mean** | *Moving abroad **means leaving** family behind.* — Mudar de país implica deixar a família. | *I didn't **mean to hurt** you.* — Não foi minha intenção te magoar. |
| **need** | *The visa form **needs checking**.* — O formulário precisa ser revisado (sentido passivo!). | *I **need to check** the visa form.* — Eu preciso revisar. |

Macete geral: **-ing olha para trás ou para a ação em si** (memória, hábito, experiência); **to olha para frente** (tarefa, intenção, propósito).

### Verbo + pessoa + to (I want YOU to do it)

Em português dizemos "quero **que você** venha" — em inglês **não existe** *want that*. A estrutura é **verbo + objeto + to + verbo**:

```
SUJEITO + VERBO + PESSOA + TO + VERBO

I    want      you   to come    to the party.
Eu   quero     que você venha   à festa.
```

| Inglês | Tradução |
|--------|----------|
| The agent **advised us to apply** early. | A agente nos aconselhou a aplicar cedo. |
| My boss **asked me to work** on Saturday. | Meu chefe me pediu para trabalhar no sábado. |
| The landlord **allowed us to keep** a cat. | O proprietário nos permitiu ter um gato. |
| **Remind me to call** the bank tomorrow. | Me lembre de ligar para o banco amanhã. |
| I **expect him to arrive** by 6. | Espero que ele chegue até as 6. |

Outros do mesmo grupo: *tell, invite, teach, encourage, warn, persuade, order, expect*.

### Make, let, help — infinitivo SEM to

Três verbos quebram a regra e usam o verbo "pelado" (bare infinitive):

- *The movie **made me cry**.* — O filme me fez chorar. (nunca ~~made me to cry~~)
- *My parents **let me travel** alone.* — Meus pais me deixaram viajar sozinha.
- *Can you **help me carry** this?* — Pode me ajudar a carregar isso? (*help me **to** carry* também é aceito)

> Na **voz passiva**, *make* recupera o *to*: *I **was made to wait** for an hour.* — Me fizeram esperar uma hora.

### Depois de preposição → SEMPRE -ing

Qualquer preposição (in, at, of, for, about, without, after, before...) força **-ing** no verbo seguinte:

| Inglês | Tradução |
|--------|----------|
| I'm **interested in learning** more about BIM. | Tenho interesse em aprender mais sobre BIM. |
| She's **good at explaining** things. | Ela é boa em explicar as coisas. |
| He left **without saying** goodbye. | Ele saiu sem se despedir. |
| **Instead of driving**, take the train to the city. | Em vez de dirigir, pegue o trem para o centro. |
| **Thanks for helping** me move. | Obrigada por me ajudar na mudança. |
| **After arriving** in Brisbane, we opened a bank account. | Depois de chegar em Brisbane, abrimos uma conta. |

Dois casos onde o **to é preposição** (e por isso vem -ing, não verbo):

- ***look forward to* + -ing**: *I'm **looking forward to meeting** you.* — Estou ansiosa para te conhecer. (nunca ~~to meet~~)
- ***be/get used to* + -ing**: *I'm **used to waking up** early.* — Estou acostumada a acordar cedo.

> ⚠️ Não confunda: ***used to* + verbo** = costumava (passado): *I **used to live** in Brazil.* · ***be used to* + -ing** = estar acostumado (presente): *I'm **used to living** in Australia.*

### Gerúndio como sujeito da frase

Quando o verbo é o **assunto** da frase, ele vira -ing (ou usa-se *It's... to*):

- ***Finding** a rental in Brisbane **is** hard.* — Encontrar aluguel em Brisbane é difícil.
- = *It's hard **to find** a rental in Brisbane.* (mesmo sentido, estrutura com *it*)
- ***Swimming** in the ocean is my favourite thing here.* — Nadar no mar é minha coisa favorita aqui.

Note o verbo no **singular** (*Finding... is*), porque o sujeito é a ação (uma coisa só).

### Infinitivo de propósito e adjetivo + to

**Para fazer algo** = *to + verbo* (nunca ~~for + verbo~~):

- *I went to the shops **to buy** groceries.* — Fui às lojas (para) comprar comida.
- *She's saving money **to visit** Brazil next year.* — Ela está economizando para visitar o Brasil.

Depois de **adjetivos**, vem infinitivo:

- *I'm **happy to help**.* — Fico feliz em ajudar.
- *English is **hard to master**, but **easy to start**.* — Difícil de dominar, fácil de começar.
- *It was **nice to meet** you.* — Foi um prazer te conhecer.

### Formas negativa, passiva e perfeita

- **Negativa**: o *not* vem **antes** — *He decided **not to go**.* / *I regret **not studying** harder.*
- **Passiva**: *She hates **being told** what to do.* — Ela odeia que digam o que fazer. / *I expect **to be contacted** by the university.* — Espero ser contatada pela universidade.
- **Perfeita** (ação anterior, estilo formal): *I'm sorry **to have missed** your call.* — Sinto muito por ter perdido sua ligação.

## Pegadinhas e erros comuns de brasileiros

1. **"Quero que você..." com that** — ❌ *I want that you come.* → ✅ *I want **you to come**.* (want NUNCA aceita "that")
2. **To depois de make/let** — ❌ *She made me to wait.* → ✅ *She made me **wait**.*
3. **Look forward to + infinitivo** — ❌ *I look forward to see you.* → ✅ *...to **seeing** you.* (esse "to" é preposição)
4. **"Para" virando for** — ❌ *I went there for buy food.* → ✅ *...**to buy** food.* (propósito = to + verbo)
5. **Sujeito sem -ing** — ❌ *Find a job is difficult.* → ✅ ***Finding** a job is difficult* (ou *It's difficult **to find** a job*).
6. **Used to trocado** — ❌ *I'm used to wake up early.* → ✅ *I'm used to **waking** up early.* (acostumado = be used to + -ing)

## Auto-teste

1. Complete: *Don't forget ______ (renew) your visa — and I'll never forget ______ (see) a kangaroo for the first time!*
2. Certo ou errado: *"My manager wants that I work this weekend."*
3. Traduza: "Estou ansiosa para conhecer a Austrália."
4. Qual a diferença? *I tried to open the window* vs *I tried opening the window.*
5. Complete: *The apartment needs ______ (clean) before the inspection, so she made us ______ (do) it today.*

<details>
<summary><strong>Respostas</strong></summary>

1. *Don't forget **to renew** your visa* (tarefa futura) — *I'll never forget **seeing** a kangaroo* (memória do passado).
2. Errado — *want* não aceita "that": *My manager wants **me to work** this weekend.*
3. *I'm looking forward **to visiting/seeing** Australia.* (to = preposição → -ing)
4. ***tried to open*** = me esforcei para abrir (estava difícil, talvez não consegui) · ***tried opening*** = experimentei abrir (como teste, para ver se resolvia algo).
5. *The apartment needs **cleaning*** (need + -ing = sentido passivo) *...she made us **do** it* (make + infinitivo sem to).

</details>

## Fontes para estudar

| Tipo | Fonte | Onde |
|------|-------|------|
| 📖 Referência | Perfect English Grammar — Gerunds and Infinitives | <https://www.perfect-english-grammar.com/gerunds-and-infinitives.html> |
| ✏️ Exercícios | Agendaweb (Verbs → gerunds and infinitives) | <https://agendaweb.org> |
| ✏️ Exercícios | ego4u — Infinitive and Gerund | <https://www.ego4u.com/en/cram-up/grammar/infinitive-gerund> |
| 📕 Livro | English Grammar in Use (Murphy, azul) — unidades sobre verb + -ing e verb + to (gerund/infinitive) | Livro |
| 📖 Consulta | Cambridge Grammar — Verb patterns | <https://dictionary.cambridge.org/grammar/british-grammar/> |
| 🎥 Vídeo EN | mmmEnglish (Emma) — Gerunds & Infinitives | YouTube: @mmmEnglish_Emma |
| 🎥 Vídeo PT-BR | Mairo Vergara — gerúndio e infinitivo em inglês | YouTube: @MairoVergara |

## Revisão rápida (5 min)

- [ ] Recite a regra de ouro: **depois de preposição, sempre -ing** (inclusive *look forward to* e *be used to*)
- [ ] Recite os 4 pares que mudam de sentido: **remember · forget · stop · try** (-ing = passado/experiência · to = tarefa/esforço)
- [ ] "Quero que você..." = **want + pessoa + to** — fale 3 frases (*I want you to..., She asked me to..., They advised us to...*)
- [ ] **make/let + verbo sem to** — fale 2 frases sobre sua rotina
- [ ] Armadilha nº 1: nunca diga *I want **that** you...* — corrija em voz alta 3 vezes
- [ ] Fale 2 frases com gerúndio como sujeito sobre Brisbane (*Living in Brisbane..., Finding a flat...*)
- [ ] Se errou os verbos básicos (+ -ing / + to) → revise [N2-07] antes de seguir
